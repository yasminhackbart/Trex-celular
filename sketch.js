...
function preload() {
  ...
  restart = loadImage ("restart (1).png")
  Game_Over = loadImage ("gameOver (1).png")
...
function draw() {
  ...
  if (modo=="início"){
    ...
    if (trex.isTouching (grupo)){modo="fim"
        }
        create_cloud ()
        if (frameCount%500==0){checkpoint.play()}
        }
   if (modo=="fim") {
   var tempo=World.seconds
   if (tempo>2){gameOver.pause()}
    ground.velocityX=0
